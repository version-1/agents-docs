...
- セキュリティ観点から、このスキルに列挙した CDN ライブラリ以外は使わない。
...
- 表にないライブラリ、フォント、アイコン CDN、analytics、tracking、外部画像 CDN は使わない。必要に見えても、まず inline SVG、標準 HTML/CSS/JavaScript、または表にあるライブラリで代替する。
...
- CDN script は上記の許可リストから必要最小限にし、読み込み理由を HTML コメントまたは Appendix に残す。
...
- 外部送信、analytics、tracking、外部フォントの読み込みは入れない。
