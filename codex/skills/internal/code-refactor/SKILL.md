...
3. quality report の `Quality Gate` がある場合は、merge 前対応を優先し、follow-up 扱いのものは今回の変更に含める価値が明確な場合だけ扱う。
4. レビュー指摘がない場合は、`code-quality-review`, `code-review`, `code-next-developer-review` の観点を使って差分中心に問題を洗い出す。
5. 必要に応じてサブエージェントを起動し、観点を分けてレビューさせる。
   - 可読性: コードの構造、命名、コメント、スタイルなどが読みやすさを妨げていないか : `code-review` の観点を使う
   - 効率性: 計算量、I/O 回数、メモリ使用量、ホットパスへの影響などが性能問題を起こしていないか : `code-review` の性能観点を使う
   - テスト容易性: テストが書きやすい構造になっているか、テストが実装詳細に依存しすぎていないか : `code-review` のテスト観点を使う
   - 保守性: 責務の分割、依存関係、テストの構造などが将来の変更を妨げていないか : `code-next-developer-review` の観点を使う
   - 品質レポート: 複雑度、責務分離、テスト容易性、技術的負債、quality gate : `code-quality-review` の観点を使う
6. 変更ごとに「リファクタ分類」から少なくとも 1 つのカテゴリを選び、必要な reference を読む。
7. 挙動維持を前提に、選んだ分類の目的に合う小さく戻しやすい単位からリファクタリングする。
8. 既存テストまたは適切な検証コマンドを実行する。実行できない場合は理由と残リスクを明記する。
9. 変更したファイル、分類ごとの要約、検証結果、残るリスクを報告する。

### Grilling Loop

Grilling Loop を明示的に指定された際は、指摘が Low だけになるまで手順を繰り返す。
（最大 5 回）

## 判断基準

- 仕様、API、データ形式、エラー挙動、ログの意味を不用意に変えない。
- quality report のスコアを上げるために挙動や公開契約を変えない。
- 大きな抽象化より、今回の課題に合う最小の分類を選ぶ。
- `Generalize` や `Replace` は、分岐や利用箇所が増え続ける根拠がある場合に限る。
- `Specialize` や `Inline` は、抽象化が読みやすさや変更容易性を下げている場合に使う。
