...
出力フォーマット型 skill の prefix は原則として `format-` を使います。

`format-*` は、既存情報を読み手に合わせた構造、テンプレート、レビュー本文、説明文、レポート形式へ整理する skill を表します。実装、レビュー判断、コマンド実行そのものを主目的にする場合は、`code-*`、`role-*`、`cmd-*` など主責務に合う prefix を優先します。
